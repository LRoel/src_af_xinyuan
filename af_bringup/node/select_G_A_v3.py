...
def make_choice():
    ...
    while not rospy.is_shutdown():
        if max_judge(amcl_pose.x_std, gps_pose.x_std) or max_judge(amcl_pose.y_std, gps_pose.y_std) or max_judge(
            ...
            if (amcl_pose.x_std > 0.224 and gps_pose.x_std > 0.5) or (amcl_pose.y_std > 0.224 and gps_pose.y_std > 0.5):
                continue
            else:
...
class Job_AMCL(threading.Thread):
    ...
    def run(self):
        cmd_state = Bool()
        cmd_state.data = True
        pub_cmd.publish(cmd_state)
        print "pub initial ok"
        self.amcl_process()
    ...
    def amcl_set_pose(self, req):
        ...
        self.amcl_nomotion_update()
        time.sleep(0.25)
        while t < 20 and not rospy.is_shutdown():
            self.amcl_nomotion_update()
            time.sleep(0.25)
    ...
    def amcl_check(self):
        if amcl_pose.x_std <= 0.224 and amcl_pose.y_std <= 0.224:
            return True
        else:
            return False
    ...
    def amcl_recovery(self):
        ...
        if odom_pose.flag_robot_move:
            ...
            while t < 15 and self.__running.isSet():
                ...
                if amcl_pose.x_std > 0.224 or amcl_pose.y_std > 0.224:
                    t += 1
                else:
                    return True
            return False
...
if __name__ == '__main__':
    time.sleep(1)
    rospy.init_node('combined_xinyuan')
    ...
    pub_cmd = rospy.Publisher('/cmd_vel/state', Bool, queue_size=1)
