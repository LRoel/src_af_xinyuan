...
:current_state_(IDLING_A), current_id_(0),
 ...
 photo_flag_(false), abnormal_flag_(false), init_flag_(false)
{
...
#define DIFF
        ...
        if (vel_msg.angular.z >= -0.20 && vel_msg.angular.z <= 0.20)
            vel_msg.angular.z = 0;
...
void AFRobot::currentResultCallback(const af_msgs::Vertex::ConstPtr& msg)
{
    ...
    string_msg.data.clear();
    string_msg.data.resize(10);
ROS_INFO("current result check***");
    switch (current_state_)
    {
    case PATROLLING_A:
    {
	std::cout << "patrol state\n";
        int intro_id = 0;
        ROS_WARN("checking %d if intro***",msg->id);
        if (isIntroSite(msg->id, &intro_id))
        {
std::cout << "vertex("<<msg->id<<") is intro & intro_id(" << intro_id<<")\n";
            string_msg.data[0] = 0x56;
...
void AFRobot::stateCallback(const af_msgs::RobotState::ConstPtr& msg)
{
    ...
    abnormal_flag_ = msg->abnormal_detection;
if(init_flag_)
{
	             std_msgs::Int8MultiArray string_msg;
            string_msg.data.clear();
       ...
       if (abnormal_flag_)
	{
	    ...
	    ab_reported = true;
	     std_msgs::Int8MultiArray string_msg;
            ...
            string_msg.data[0] = 0x56;
            string_msg.data[1] = 0x03; // 触发状态
            string_msg.data[2] = 03; // yixiangjiance
            string_msg.data[9] = checksum(string_msg);
            android_pub_.publish(string_msg);
	}
	else
    ...
    do
    {
        ...
        if (battery_percentage_ >= HIGH_BATTERY)
        {
