...
{
    std::cout << "checking if intro Vertex(" << id << ")\n";
    std::auto_ptr< sql::PreparedStatement >  pstmt;
    std::auto_ptr< sql::ResultSet > res;
std::cout << "****selecting" << std::endl;
    ...
    res.reset(pstmt->executeQuery());
std::cout << "exe select * from intro_site where vid="<< id << "***\n";
    if (res->rowsCount() == 0)
    {
        std::cout << "Vertex(" << id << ") is not an intro site. Returning -1." << std::endl;
        return false;
    }
std::cout << "intro_id != null is "<<(intro_id != NULL) << std::endl;
    if (intro_id != NULL)
    {
        while (res->next())
        {
            *intro_id = res->getInt("intro_id");
        std::cout << "*intro_id=" << *intro_id << std::endl;
        }
    }
    return true;
}
